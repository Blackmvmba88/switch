...
print_status() {
  ...
  elif has_app "ChatGPT Atlas"; then
    echo "OK  Atlas instalado: /Applications/ChatGPT Atlas.app"
  else
    echo "NO  Chrome/Atlas no esta instalado."
  fi
...
open_chrome() {
  local browser_app
  browser_app="$(pick_browser_app || true)"
  if [[ -z "${browser_app}" ]]; then
    echo "No encontre Google Chrome ni ChatGPT Atlas."
    exit 1
  fi
...
case "${1:-}" in
  ...
  "")
    ...
    echo "  3. Add Non-Steam Game -> Google Chrome o Atlas"
    echo "  4. Lanza el navegador desde Steam Big Picture si aparece en Library"
    echo
