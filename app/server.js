...
const MODE_STATUS = path.join(APP_ROOT, "reports/control-room-mode.json");
...
const COMMANDS = {
  ...
  "atlas": { args: ["atlas"], timeout: 15000 },
  "atlas-status": { args: ["atlas-status"], timeout: 15000 }
};
...
function runBmctl(name) {
  ...
  if (!spec) {
    return Promise.resolve({ ok: false, code: 404, stdout: "", stderr: `Unknown command: ${commandName}` });
  }
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "GET" && url.pathname === "/api/mode") {
      sendJson(res, 200, { ok: true, mode: readJson(MODE_STATUS) || { mode: "game" } });
      return;
    }
    if (req.method === "POST" && url.pathname === "/api/mode") {
      const payload = await readBody(req);
      const mode = String(payload?.mode || "");
      if (!["game", "manual", "fun"].includes(mode)) {
        sendJson(res, 400, { ok: false, error: "invalid mode" });
        return;
      }
      writeJson(MODE_STATUS, { mode, updatedAt: new Date().toISOString() });
