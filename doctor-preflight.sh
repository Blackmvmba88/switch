...
if [[ "${OS}" != "Darwin" ]]; then
  warn "Este runtime esta diseñado principalmente para macOS. Algunos checks fallaran en ${OS}."
fi
...
if [[ "${OS}" == "Darwin" ]]; then
  ...
  warn "Skipping Google Chrome /Applications check (non-macOS)"
fi
...
if command -v swiftc >/dev/null 2>&1; then
  ...
  if [[ "${OS}" == "Darwin" ]]; then
    warn "swiftc no disponible; HID native source requiere Xcode Command Line Tools"
  else
    warn "swiftc no disponible; HID native source compilation only supported on macOS"
  fi
fi
...
if [[ "${OS}" == "Darwin" ]]; then
  ...
  warn "Skipping USB check (non-macOS system_profiler)"
fi

mkdir -p "${APP_ROOT}/logs" "${APP_ROOT}/reports" "${APP_ROOT}/runtime" "${APP_ROOT}/profiles" 2>/dev/null || true
if [[ -w "${APP_ROOT}" ]]; then
  pass "Application Support escribible: ${APP_ROOT}"
else
  warn "Application Support no escribible: ${APP_ROOT} (esto es normal fuera de macOS/entornos restringidos)"
fi
...
if [[ "${OS}" == "Darwin" ]]; then
  ...
  warn "Skipping LaunchAgents check (non-macOS)"
fi
...
if [[ "${FAILED}" == "1" ]]; then
  if [[ "${OS}" != "Darwin" ]]; then
    echo "Preflight encontro bloqueadores. Nota: estas en ${OS} y este software es para macOS."
  else
