...
if command -v swiftc >/dev/null 2>&1; then CLANG_MODULE_CACHE_PATH="${TMP_DIR}/clang-module-cache" swiftc "${ROOT}/runtime/hid-raw-monitor.swift" -o "${TMP_DIR}/hid-raw-monitor"; else echo "WARN: swiftc not found, skipping native compilation check"; fi
