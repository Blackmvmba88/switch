...
const repoProfile = path.join(root, "profiles", "rock-candy-wired-controller-for-nintendo-switch-vendor-0e6f-product-0187.normalized.json");
const appProfile = path.join(process.env.HOME, "Library/Application Support/BlackMambaInput/profiles/rock-candy.normalized.json");
const files = [repoProfile];
if (fs.existsSync(appProfile)) files.push(appProfile);
...
function main() {
  ...
  if (!fs.existsSync(appProfile)) {
    fs.mkdirSync(path.dirname(appProfile), { recursive: true });
    const profile = applyLayout(JSON.parse(fs.readFileSync(repoProfile, "utf8")), mode);
    profile.updatedAt = new Date().toISOString();
    fs.writeFileSync(appProfile, JSON.stringify(profile, null, 2) + "\n");
    console.log(`created ${appProfile}`);
  }
